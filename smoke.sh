...
EXPECTED_VERSION="$(/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' "$ROOT/Info.plist" 2>/dev/null || true)"
VERSION="$(/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' "$APP/Contents/Info.plist" 2>/dev/null || true)"
[[ -n "$EXPECTED_VERSION" && "$VERSION" == "$EXPECTED_VERSION" ]] && pass "Planner version $EXPECTED_VERSION" || bad "Planner version matches source ($EXPECTED_VERSION)"
